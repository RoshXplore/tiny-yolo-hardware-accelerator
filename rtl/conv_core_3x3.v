module conv_core_3x3 #(
    ...
    parameter QW = 16,      // Quantized Weight Width (16-bit usually better for accuracy)
    parameter ACC_W = 32    // Accumulator Width (Prevent overflow)
)(
    input wire clk,
    input wire rst,
    input wire valid_in,    // From Line Buffer's "window_valid"

    // 1. The 9 Pixels from Line Buffer
    // We treat them as SIGNED for safe math, even if pixels are 0-255.
    // We will zero-extend them before math.
    ...
    always @(posedge clk) begin
        // Note: $signed({1'b0, p00}) converts unsigned 8-bit to positive signed
